...
function __fargate_completion_certificate {
  certificates=($(fargatecli certificate list | tail -n +2 | cut -f1 | sed "s_*_\\\*_g"))
...
function __fargate_completion_service {
  services=($(fargatecli service list | tail -n +2 | cut -f1))
  [ -z "$services" ] || _values 'service' $services
}

function __fargate_completion_task {
  tasks=($(fargatecli task list | tail -n +2 | cut -f1))
  [ -z "$tasks" ] || _values 'task' $tasks
}

function __fargate_completion_taskid {
  taskids=($(fargatecli task ps test | tail -n +2 | cut -f-2,4 | sed -e 's_:_\\:_' -e 's_\t_:_' -e 's_\t_(_' -e 's_$_)_'))
  [ -z "$taskids" ] || _describe 'taskid' taskids
 }

function __fargate_completion_loadbalancer {
  loadbalancers=($(fargatecli lb list | tail -n +2 | cut -f1))
...
function __fargate_completion_env {
  keys=($(fargatecli service env list $line | sed 's_=.*__'))
